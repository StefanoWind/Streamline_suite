...
Inputs:
    ...
    dt: [s] timestep of simulator
    ang_tol: [deg] angular tolerance
    azi0: [deg] start azimuth
    ele0: [deg] start elevation
    ppd1: [pulses/degree] firing rate in azimuth (see Halo manual)
    ppd2: [pulses/degree] firing rate in elevation (see Halo manual)
"""
...
def Halo_scan_sim(scan_file,ppr,identifier,model,source_time,dt=0.01,ang_tol=0.25,azi0=0,ele0=0, ppd1=500000/360,ppd2=250000/360):
    ...
    for i in range(len(P1)):
        ...
        dazi_acc=np.min([s1[i]**2/(2*a1[i]+10**-10),np.abs(azi_end-azi_srt)/2])#azimuth span to accelerate/decelerate
        sign_azi=(azi_end-azi_srt)/np.abs(azi_end-azi_srt+10**-10)#azimuth direction
        
        dele_acc=np.min([s2[i]**2/(2*a2[i]+10**-10),np.abs(ele_end-ele_srt)/2])#elemuth span to accelerate/decelerate
        ...
        #sampling times
        time=np.arange(t[0],t[-1],tau_s)
        
        #append to global trajectory
        time_all=np.append(time_all,time_all[-1]+time)
        azi_all=np.append(azi_all,np.interp(time,t,azi))
        ele_all=np.append(ele_all,np.interp(time,t,ele))
        
        #append end point
        time_all=np.append(time_all,time_all[-1]+tau_s)
    ...
    return time_all,azi_all,ele_all
